...
let page = 0;
let debounceID;
let category_filter = JSON.parse(localStorage.getItem("category_filter")) || [
  false,
  false,
  false,
];
  ...
  if (BrandSearch != undefined) {
    data = data.filter((el) => {
      if (el.title != null && el.subtitle != null) {
        return (
          el.subtitle.toUpperCase().includes(BrandSearch.toUpperCase()) ||
          el.title.toUpperCase().includes(BrandSearch.toUpperCase())
        );
      }
    });
  }
      ...
      if (el.strike_price != null) {
        ...
        div.onclick = () => {
          localStorage.setItem(
            "SimilarProducts",
            JSON.stringify(
              similarProducts(el.tilte, el.subtitle, data.slice(i + 1))
            )
          );
