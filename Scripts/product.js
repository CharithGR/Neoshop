...
window.onload = () => {
  ...
  Add_to_Cart.className = "edit_add_to_cart_btn";
  Add_to_Cart.innerHTML = `<div> <div><svg
...
>
  ...
  <path
    ...
    fill="white"
  ></path>
</svg></div><div>Add to cart</div></div>`;
  Add_to_Cart.onclick = () => {
  ...
  details_p.className = "edit_delivery_creds";
  ...
  cart_wish_div.append(Add_to_Cart, wishlist);
  ...
  let a = cart_data.filter((el) => {
    if (
      ...
      if (el.Quantity > 5) {
        el.Quantity = 5;
        alert(
          "Product already in cart, Quantity increased to 5.You cannot buy more than 5 items of this product"
        );
      } else {
